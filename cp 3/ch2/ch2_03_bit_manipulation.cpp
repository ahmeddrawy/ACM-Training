...
int main() {
  ...
  int i =0;             // T will always be a n raised to power (index) int this case indx = 0
                        // divide by 2 as you move on and the result -1 is the indx
  while(T>>i)   i++;
  cout<<i-1<<endl;
  S = 52; printSet(S);
  T = lowBit(S); printf("T = %d (this is always a power of 2)\n", T);
  ...
  for (int i = 0; i <= 16; i++)
   ...
   printSet(40); cout<<endl;
  printf("S = %d, turn off last bit in S, S = %d\n", 40, turnOffLastBit(40));
  printSet(turnOffLastBit(40));cout<<endl;
  printSet(41);cout<<endl;


  printf("S = %d, turn on last zero in S, S = %d\n", 41, turnOnLastZero(41));
  printSet(turnOnLastZero(41));cout<<endl;
